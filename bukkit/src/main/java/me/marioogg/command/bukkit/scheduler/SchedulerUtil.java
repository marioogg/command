...
public class SchedulerUtil {
    ...
    private static final Logger logger = BukkitCommandHandler.getLogger();
    ...
    public static void runAsync(Runnable task) {
        if (FOLIA) {
            try {
                ...
                logger.log(Level.SEVERE, "Error running an asynchronous task for folia:", e);
            }
        } else {
    ...
    public static void runSync(Runnable task) {
        if (FOLIA) {
            try {
                ...
                logger.log(Level.SEVERE, "Error running a synchronous task for folia:", e);
            }
        } else {
    ...
    public static void runSyncForEntity(Entity entity, Runnable task) {
        if (FOLIA) {
            try {
                ...
                logger.log(Level.SEVERE, "Error running a synchronous task (entity specific) for folia:", e);
            }
        } else {
            Bukkit.getScheduler().runTask(BukkitCommandHandler.getPlugin(), task);
        }
