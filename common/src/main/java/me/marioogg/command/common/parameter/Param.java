package me.marioogg.command.common.parameter;

import java.lang.annotation.ElementType;
import java.lang.annotation.Retention;
import java.lang.annotation.RetentionPolicy;
import java.lang.annotation.Target;

/**
 * Declares command parameter metadata for all platforms.
 */
@Retention(RetentionPolicy.RUNTIME)
@Target(ElementType.PARAMETER)
public @interface Param {
    /**
     * Name of the parameter.
     */
    String name();

    /**
     * Makes it so the rest of arguments in a command are connected together
     * after a concated parameter is found.
     */
    boolean concated() default false;

    /**
     * Checks whether a parameter is required.
     */
    boolean required() default true;

    /**
     * Used as the default value if required is false and no parameter parse is provided.
     */
    String defaultValue() default "";
}

